...
- **Run**: `npm run test:e2e` (requires Xvfb + herbstluftwm on Linux CI, or native display on macOS). On local Wayland, `scripts/run-e2e-isolated.mjs` starts an Xvfb display using `-displayfd`, removes inherited Wayland socket access, and fails before launching Obsidian if Xvfb is unavailable. The Nix dev shell provides Xvfb; outside Nix, install it or set `MOTIONS_XVFB_BIN` to its executable path.
