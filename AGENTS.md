...
- **`npm run verify` runs all six static gates**: `typecheck` (`tsc -noEmit`), `typecheck:tests`, `lint`, `format:check` (Prettier, pinned as a devDependency), `lint:patterns` (`ast-grep scan`), and `lint:deadcode` (`knip`). All six are blocking in CI.
- **`typecheck:tests` exists because `test/` is outside `tsconfig.json`'s `src/**/*.ts` include, so nothing type-checked it.** A broken import passed typecheck, lint and format alike and surfaced only when the spec ran: a missing `getVimMode` import replaced a real macOS failure with a `ReferenceError`, an import of `useSourceProperties` that `helpers` never exported was invisible, and a `node:path` call inside a browser callback failed as `resolvePath is not defined`. Full strict checking of `test/` is not yet in place, but it is closer than it looks: with `mocha` and `vitest/globals` declared, strict reports **240** errors, not the ~4100 seen before those globals were added. About 100 are nullability (`TS18048`, `TS2532`, `TS2722`, mostly `plugin` and `el` after a lookup), 47 argument-type, 30 missing-property, 16 implicit-any. Only 14 are in `src/`, and those are configuration gaps rather than defects: `__DEV__` needs a global declaration, `./companion.lua` the text-loader `d.ts`, and `./bundled/global.json` needs `resolveJsonModule`. The vendored fengari sources report **zero** under strict; the two they report under `strict: false` are a null-narrowing artifact of relaxing the flag, so typing them would fix nothing. Until that cleanup happens, `scripts/typecheck-tests.mjs` reports only resolution failures (`TS2304`, `TS2305`, `TS2307`, `TS2551`, `TS2552`) against `tsconfig.test.json`. Those are unaffected by strictness and are the class that actually broke. It does **not** catch a scope error like the `resolvePath` one, which is valid TypeScript in the file and wrong only at runtime.
