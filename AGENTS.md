...
The desktop `GlobalKeyHandler` also owns the native File Explorer's fixed contextual `h`/`j`/`k`/`l` aliases when workspace navigation is enabled. It tracks pointer, focus, and active-leaf interactions because Obsidian can send tree keystrokes to `BODY`, and clears that context when focus leaves the explorer. It honors numeric prefixes, preserves pending chords such as `<C-w>h`, translates only unmodified, non-composing events outside editors, modals, inputs, and contenteditable controls into same-document arrow events, and suppresses those synthetic events from the physical-key observer before allowing Obsidian's native tree handler to consume them.
...
- **Run**: `npm run test:e2e` (requires Xvfb + herbstluftwm on Linux CI, or native display on macOS). On local Wayland, `scripts/run-e2e-isolated.mjs` starts an Xvfb display, removes Wayland socket access from the test process, and fails without opening Obsidian if Xvfb is unavailable. It accepts `MOTIONS_XVFB_BIN` or an executable cached at `.obsidian-cache/tools/Xvfb`.
